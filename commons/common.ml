...
(* more general, handle multilines *)
let md5sum_of_multi_line_string s =
  let tmpfile = new_temp_file "md5sum" "x" in
  write_file ~file:tmpfile s;
  let com = spf "cat %s | md5sum | cut -d\" \" -f 1" tmpfile
  in
  match cmd_to_list com with
  | [s] -> 
      (*pr2 s;*)
      s
  | _ -> failwith "md5sum_of_multi_line_string wrong output"
