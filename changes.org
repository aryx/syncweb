...
** features:
- add a syncweb -no_cache to disable the use of .xxx.nwcache files
  (which can cause pbs when syncing the same files but from different dirs
   like for principia and goken)

* 0.9 (Q1 2026) (syncweb -loc for LOC/LOE stats, configure --syncweb-only)
# start to use Claude

** features:
- new syncweb -loc action to give statistics about the .nw file such
  as the lines of code (LOC) and lines of explanations (LOE).
...
- allow [<parse()>] to be used instead of [<Parse.parse()>]
  when no ambiguity
